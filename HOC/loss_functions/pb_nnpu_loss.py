...
class _nnPULoss(Function):
    ...
    @staticmethod
    def forward(ctx, pred, label, prior, beta, gamma):
        # Save parameter for backward
        ctx.save_for_backward(pred, label, prior, beta, gamma)
        # Loss function
        ctx.loss_func = lambda x: torch.sigmoid(-x)
        # Get the Loss positive and negative samples
        ctx.positive_x = (label == 1).float()
        ctx.unlabeled_x = (label == 0).float()
    ...
    def backward(ctx, grad_output):
        pred, label, prior, beta, gamma = ctx.saved_tensors
...
class NnPULossPb(nn.Module):
    def __init__(self, prior, beta=None, gamma=None):
        super(NnPULossPb, self).__init__()
    ...
    def forward(self, pred, label, epoch, device):
        return _nnPULoss.apply(pred, label, self.prior.cuda(), self.beta.cuda(), self.gamma.cuda()), torch.tensor(0), torch.tensor(0)
