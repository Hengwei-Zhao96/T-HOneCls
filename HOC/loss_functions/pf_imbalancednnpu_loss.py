...
class _ImbalancednnPULoss(Function):
    @staticmethod
    def forward(ctx, pred, positive_mask, unlabeled_mask, prior, beta, gamma):
        ...
        ctx.positive_loss = torch.sum(ctx.positive_x / ctx.positive_num * ctx.positive_y.squeeze())
    ...
    def backward(ctx, grad_output):
        ...
        d_positive_loss = -ctx.positive_x / ctx.positive_num * ctx.positive_y.squeeze() * (
                1 - ctx.positive_y.squeeze())
        d_negative_loss = (
                                  ctx.unlabeled_x / ctx.unlabeled_num - prior * ctx.positive_x / ctx.positive_num) * ctx.unlabeled_y.squeeze() * (
                                  1 - ctx.unlabeled_y.squeeze()) / (1 - prior)
...
class ImbalancedNnPULossPf(nn.Module):
    def __init__(self, prior, beta=None, gamma=None):
        super(ImbalancedNnPULossPf, self).__init__()
    ...
    def forward(self, pred, positive_mask, unlabeled_mask, epoch, device):
        return _ImbalancednnPULoss.apply(pred, positive_mask, unlabeled_mask, self.prior.cuda(), self.beta.cuda(),
