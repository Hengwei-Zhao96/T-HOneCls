...
class OsKLLoss(nn.Module):
    def __init__(self):
        super(OsKLLoss, self).__init__()

    def forward(self, os_target_s, os_target_t):
        target_s = os_target_s.squeeze()
        target_t = os_target_t.squeeze()
        kl_loss = 0.0
        for i in range(target_s.shape[0]):
            pred1_normalized = logits_norm(target_s[i]) + 1e-8
            pred2_normalized = logits_norm(target_t[i]) + 1e-8

            kl_loss += (nn.KLDivLoss(reduction='none')(torch.log(pred1_normalized), pred2_normalized) + nn.KLDivLoss(
                reduction='none')(torch.log(pred2_normalized), pred1_normalized)).mean()

        kl_loss = kl_loss / target_s.shape[0]
        return kl_loss
