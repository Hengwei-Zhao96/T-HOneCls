...
from HOC.models.seblock import SEBlock
...
@MODELS.register_module()
class OSFreeNet(nn.Module):
    def __init__(self, in_channels, num_classes, block_channels, num_blocks, inner_dim, reduction_ratio):
        super(OSFreeNet, self).__init__()
        r = int(16 * reduction_ratio)
        block1_channels = int(block_channels[0] * reduction_ratio / r) * r
        block2_channels = int(block_channels[1] * reduction_ratio / r) * r
        block3_channels = int(block_channels[2] * reduction_ratio / r) * r
        ...
        ####OS Decoder
        self.os_reduce_1x1convs = nn.ModuleList([
            nn.Conv2d(block1_channels, inner_dim, 1),
            nn.Conv2d(block2_channels, inner_dim, 1),
            nn.Conv2d(block3_channels, inner_dim, 1),
            nn.Conv2d(block4_channels, inner_dim, 1),
        ...
        self.os_fuse_3x3convs = nn.ModuleList([
            nn.Conv2d(inner_dim, inner_dim, 3, 1, 1),
            nn.Conv2d(inner_dim, inner_dim, 3, 1, 1),
            nn.Conv2d(inner_dim, inner_dim, 3, 1, 1),
            nn.Conv2d(inner_dim, inner_dim, 3, 1, 1),
        ...
        self.os_cls_pred_conv = nn.Conv2d(inner_dim, num_classes, 1)
    ...
    def forward(self, x, y=None, w=None, **kwargs):
        ...
        #### OS
        os_inner_feat_list = [self.os_reduce_1x1convs[i](feat) for i, feat in enumerate(feat_list)]
        os_inner_feat_list.reverse()

        os_out_feat_list = [self.os_fuse_3x3convs[0](os_inner_feat_list[0])]
        for i in range(len(os_inner_feat_list) - 1):
            os_inner = self.top_down(os_out_feat_list[i], os_inner_feat_list[i + 1])
            os_out = self.os_fuse_3x3convs[i + 1](os_inner)
            os_out_feat_list.append(os_out)

        final_feat = out_feat_list[-1]
        logit = self.cls_pred_conv(final_feat)

        os_final_feat = os_out_feat_list[-1]
        os_logit = self.os_cls_pred_conv(os_final_feat)
        return logit, os_logit
