...
from deprecated_files.deep_pu_calibration import PUCalibration
import os
...
if __name__ == "__main__":
    ...
    model = FreeNetEncoder(in_channel=image_data.shape[0], out_channels=1, patch_size=patch_size)
    optimizer = torch.optim.SGD(params=model.parameters(),
                                momentum=config['optimizer']['params']['momentum'],
                                weight_decay=config['optimizer']['params']['weight_decay'],
                                lr=config['learning_rate']['params']['base_lr'])
    pu_estimator = PUCalibration(estimator=model, epoch=1, optimizer=optimizer,mode=mode)
    pu_estimator.fit(positive_data=positive_train_data, unlabeled_data=unlabeled_train_data)
